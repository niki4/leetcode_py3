    ...
    def search(self, nums: List[int], target: int) -> bool:
        ...
        while left <= right:
            ...
            if nums[left] < nums[mid]:
                # target is in strictly increasing part in left subarr
                if nums[left] <= target < nums[mid]:
                    right = mid - 1
                else:
                    left = mid + 1
            elif nums[left] > nums[mid]:
                # target is in strictly increasing part in right subarr
                if nums[mid] < target <= nums[right]:
                    left = mid + 1
                else:
                    right = mid - 1
            else: # cannot define subarr (dup value?)
                left += 1
...
if __name__ == '__main__':
    ...
    for s in solutions:
        for inp, trg, exp in tc:
            ...
            assert res is exp, f'{s.__class__.__name__}: want {exp}, got {res}'
